...
export function SidebarRecentSessions() {
  ...
  const emailSubjects = emailQueries.map((q) => (q.data as Record<string, unknown>)?.subject as string ?? "")
  const taskTitles = taskQueries.map((q) => (q.data as Record<string, unknown>)?.title as string ?? "")
