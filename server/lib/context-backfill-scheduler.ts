...
export async function runCuratedUpdate(
  ...
  const relPath = plugin.backfillDir ?? `plugins/context/assets/${plugin.id}`
  ...
  // Paths in the batch are workspace-relative (e.g. "plugins/context/assets/gmail/abc.md"
  ...
  const filePaths = batch.map((f) => (
    f.path.startsWith("plugins/context/assets/")
      ? f.path.slice("plugins/context/assets/".length)
      : `../${f.path}`
  ))
...
- **Tier 3** (schema/template/prompt changes) — write a row to \`plugins/context/assets/proposals.md\` instead of editing code; operator review required.
...
Flat format (relative to \`plugins/context/assets/\`):
