...
/** Same shape as `buildPluginContext` itself (derived, not hand-copied, so it
 *  can't drift) — tests substitute a fake here instead of mocking
 *  plugin-context.js. Also the shape the `getContext` helper passed to a
 *  plugin's custom `routes()` below builds from. */
type GetPluginContext = typeof buildPluginContext

/**
 * Builds the auto-generated item routes for all plugins. `getContext` defaults
 * to the real `buildPluginContext` (which resolves credentials over the
 * network); tests pass a fake so route behavior is exercised without mocking
 * the plugin-context module.
 */
export function createPluginRoutes(getContext: GetPluginContext = buildPluginContext): Hono<AppBindings> {
  const pluginRoutes = new Hono<AppBindings>()
