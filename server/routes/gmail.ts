...
import { cached, get as getCached, set as setCached, invalidate } from "../lib/cache.js"

const LIST_TTL = 300_000 // 5 min for message lists
...
gmailRoutes.get("/messages", async (c) => {
  ...
  const listKey = `gmail:messages:${query}:${max}:${pageToken || ""}`

  // 1. Standard cache hit
  const cachedResult = getCached<ListResult>(listKey)
  if (cachedResult) return c.json(cachedResult)

  // 2. Incremental sync (first page only — paginated pages always do full fetch)
  if (!pageToken) {
    ...
    if (syncState?.historyId) {
      try {
        ...
        if (changedThreadIds.size === 0) {
          ...
          setCached(listKey, result, LIST_TTL)
  ...
  setCached(listKey, response, LIST_TTL)
...
gmailRoutes.get("/messages/:id", async (c) => {
  ...
  const message = await cached(`gmail:message:${id}`, DETAIL_TTL, () => gmail.getMessage(id))
  return c.json(message)
})

gmailRoutes.get("/threads/:id", async (c) => {
  const id = c.req.param("id")
  const thread = await cached(`gmail:thread:${id}`, DETAIL_TTL, () => gmail.getThread(id))
  return c.json(thread)
})

gmailRoutes.get("/labels", async (c) => {
  const result = await cached("gmail:labels", LABEL_TTL, () => gmail.getLabels())
  return c.json(result)
})
