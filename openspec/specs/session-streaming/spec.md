...
- **THEN** the client MUST reconnect with delays `1s, 2s, 4s, 8s, 16s, 32s, 64s` (seven retries, doubling from `WS_RECONNECT_INITIAL_DELAY_MS`, capped at `WS_RECONNECT_MAX_DELAY_MS` = 64s), each passed through full jitter — the actual wait is uniformly distributed between `0` and the listed value.
- **AND** after the seventh retry fails (the eighth connection attempt overall, counting the initial one), the client stops scheduling further reconnects and the connection store records `reconnectPhase: "exhausted"` — the UI surfaces this as a terminal state rather than an indefinitely-spinning "reconnecting" toast.
- **WHY:** the client and the `ws-connection-store` UI state must agree on the same bounded schedule — a client that keeps retrying past what the store considers exhausted (or vice versa) desyncs what the UI reports from what the socket is actually doing.
- **WHY:** the jittered delay is drawn exactly ONCE per disconnect, inside `applyDisconnect`, and stored as `status.nextRetryAt`. The client's reconnect timer reads that stored value (`nextRetryAt - now`) instead of calling the backoff calculator a second time — a second call would draw a different random jitter value and fire at a different instant than the one the UI displays as "next retry at …".

#### Scenario: A superseded socket's events are ignored
- **WHEN** React StrictMode double-invokes the mounting effect on the same `WsStreamProvider` instance — mount, cleanup, mount again — and a socket from the first invocation is still open or connecting when the cleanup runs
- **THEN** the client MUST defer that socket's close until it opens (calling `close()` on a `CONNECTING` socket throws) rather than closing it immediately, so the deferred close's `onopen`/`onmessage`/`onerror`/`onclose` events can still arrive later, after the second invocation's socket has replaced it as `wsRef.current`.
- **AND** each of those four handlers MUST return immediately, before any other effect, when the socket they were installed on is no longer `wsRef.current` — checked by reference identity, not a boolean "still mounted" flag, since `mountedRef.current` is back to `true` by the time these belated events fire on the live, remounted instance.
- **AND** an ignored `onclose` MUST NOT call `recordClosed`, flip `useWsConnectionStore`'s phase to `"disconnected"`, or schedule a reconnect timer — the live socket's own connection state is authoritative and must not be overwritten by a stale one.
- **WHY** Without the identity check, the superseded socket's belated `onclose` looks, to the store, exactly like the live connection dropping: it flips the shared connection store to `"disconnected"` (surfacing a spurious "Reconnected" toast on a perfectly healthy fresh page load) and schedules a second, redundant reconnect timer alongside the live socket's own. From then on two sockets live side by side, every real server drop fires two `onclose` handlers milliseconds apart, and because the store counts every `connect()` call toward `reconnectAttemptCount`, the seven-retry budget drains in four rounds instead of seven.
...
| Requirement | Implementation |
|---|---|
| Resubscribe with cursors on reconnect | [`src/hooks/use-ws-stream.tsx:109-138`](../../../src/hooks/use-ws-stream.tsx#L109-L138) — subscribe frame builder; [`:214-226`](../../../src/hooks/use-ws-stream.tsx#L214-L226) — `connected` message handler |
...
| Bounded exponential backoff (via the shared `backoffDelayMs`), stops after exhaustion | [`src/hooks/use-ws-stream.tsx:263-273`](../../../src/hooks/use-ws-stream.tsx#L263-L273) — reads `status.nextRetryAt` and schedules the timer off it; [`src/stores/ws-connection-store.ts`](../../../src/stores/ws-connection-store.ts) — `applyDisconnect`/`nextRetryIndex`/`getWsReconnectDelayMsForRetry` compute and store `nextRetryAt` exactly once per disconnect, `WS_RECONNECT_MAX_RETRIES` |
| A superseded socket's events are ignored | [`src/hooks/use-ws-stream.tsx:169-259`](../../../src/hooks/use-ws-stream.tsx#L169-L259) — `isSuperseded()` (`wsRef.current !== ws`) guard at the top of `onopen`/`onmessage`/`onerror`/`onclose`; [`:297-318`](../../../src/hooks/use-ws-stream.tsx#L297-L318) — cleanup's deferred-close comment explains why a stale socket's events can still fire |
| cursor_miss handling | [`src/hooks/use-ws-stream.tsx:228`](../../../src/hooks/use-ws-stream.tsx#L228) — frame dispatch; [`src/stores/session-recovery.ts:154`](../../../src/stores/session-recovery.ts#L154) — invalidateBootstrap |
...
- Reconnect + superseded-socket unit tests: `src/hooks/__tests__/use-ws-stream.test.tsx` — including `Scenario: A superseded socket's events are ignored`, which mounts inside `React.StrictMode` to reproduce the double-invoke and asserts a stale socket's belated events can't reach the connection store or schedule a reconnect.
...
| Date | Commit | Change |
|------|--------|--------|
| 2026-09-02 | _pending_ | Fixed a duplicate-socket bug under React StrictMode: the mount effect's cleanup defers closing a still-connecting socket until it opens (`ws.onopen = () => ws.close()`), but left `onclose`/`onerror`/`onmessage` attached — so when StrictMode remounted the same `WsStreamProvider` instance, the superseded socket's belated `onclose` fired against the now-live instance (`mountedRef.current` was `true` again), flipping the shared connection store to `"disconnected"` (a spurious "Reconnected" toast on a healthy fresh load) and scheduling a second, redundant reconnect timer. From then on two sockets ran forever, every server drop fired two `onclose` handlers milliseconds apart, and the store's `reconnectAttemptCount` — incremented on every `connect()` call, including the duplicate's — drained the seven-retry budget in four rounds instead of seven. Fixed by checking `wsRef.current !== ws` (reference identity, not a boolean flag) at the top of all four handlers, so a superseded socket's events are silently ignored once the next `connect()` has replaced it. No interval leak results: the deferred close overwrites `onopen` before it ever fires, so the superseded socket's keepalive `setInterval` is simply never started. |
| 2026-09-02 | _pending_ | Fixed a desync between `use-ws-stream.tsx`'s reconnect timer and `ws-connection-store.ts`'s displayed retry state: the hook computed its own unbounded `1s×2^n capped at 30s` schedule and retried forever, while the store separately modeled a bounded seven-retry schedule (1s…64s, itself rebuilt on `@hammies/contracts/retry`'s `backoffDelayMs`) with a terminal `reconnectPhase: "exhausted"` the hook never reached or honored. First pass had the hook call the store's `getWsReconnectDelayMsForRetry` a second time to get its own timer delay — caught in review, because that calculator jitters randomly, so a second independent call draws a DIFFERENT delay than the one already stored in `nextRetryAt` for the UI. Final fix: `applyDisconnect` draws the jittered delay exactly once per disconnect and stores it as `nextRetryAt`; the hook derives its `setTimeout` delay from that stored timestamp instead of recomputing, and stops scheduling once it's `null` (exhausted). `getWsUiState` gained an `"exhausted"` state and `SessionConnectionSurface` surfaces it with a manual "Reload" action, since reconnects genuinely stop now instead of continuing forever. |
