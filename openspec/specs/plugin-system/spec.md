...
- 2026-09-03: `decodeIframeMessage`'s result followed `@hammies/contracts`' `IframeMessageDecodeResult<T>` onto the shared `Result` type. `PluginFrame.tsx` reads `decoded.ok`/`decoded.value` instead of `decoded.success`/`decoded.data`; behavior is unchanged.
