...
The display title is resolved from Claude Code's own JSONL metadata — `custom-title` (the editable, cross-tool title) → `ai-title` (Claude Code's auto-generated title) → `last-prompt` (the latest user prompt, far better than the first message, which for role-primed sessions is the long system prompt) → first real prompt → SDK result. `custom-title` + `last-prompt` are appended throughout the file and last-wins. The JSONL is the title authority: both the session list **and** the single-session detail route (`GET /api/sessions/:id`) prefer the JSONL-resolved title (`agentSession.summary` from `extractSessionMeta`) over the cached DB `summary`, so the list and the open session's header never disagree (a stale pre-rename DB `summary` would otherwise show only in the detail). Renames/auto-names write the title back to the JSONL (`writeCustomTitle`) so Studio and Claude Code show it too.
...
- **THEN** `title` is the latest `custom-title` if present, else the latest `ai-title`, else the latest `last-prompt`, else the first real prompt, else the SDK result.
...
- The single-session detail route (`GET /api/sessions/:id`) returned the raw DB `summary` while the list returned the JSONL-resolved title, so a session renamed in the JSONL (`custom-title`) showed the new name in the list but a stale name in the open session's header. Fixed by having the detail route prefer `agentSession.summary` (it already fetched `agentSession` for the transcript) — the JSONL is the single title authority for both surfaces.
- Title resolution gained Claude Code's `ai-title` (auto-generated) between `custom-title` and `last-prompt`, matching Studio so the two apps title the same session identically.
- Archive state became cross-tool: `extractSessionMeta` reads an `archived` JSONL entry (last-wins) which overrides the DB status in the list (so a session archived in Studio/Claude Code is hidden in Inbox too), and `archiveSession`/`unarchiveSession` append the flag (`writeArchivedFlag`) so an Inbox archive is seen by the other tools. The DB status remains the in-app authority; the JSONL flag is the shared one.
- `resumeSessionQuery` un-archives on resume: when the prior status was `archived` it writes a fresh `archived:false` flag (`writeArchivedFlag`) before setting `running`. An actively-used session can't stay archived, and the freshest write can't be buried beneath later content (the failure mode where a long resumed session's stale `archived:true` scrolled past the tail window).
