...
| Store configured + `withAdvisoryLock` impl (dedicated connection) at startup, bound to the **vault pool** (`STUDIO_DATABASE_URL`, via `getVaultPool`/`vaultQuery`) | `server/index.ts` `configureCredentialStore(...)` + `server/db/pool.ts` vault pool (files owned by `health-rate-limit-logging`) |
