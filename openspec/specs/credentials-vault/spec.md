...
> Refresh + the advisory-lock serialization moved to `@hammies/auth` ([`credential-vault`](../../../../auth/openspec/specs/credential-vault/spec.md), with tests). Inbox's role is to (a) wire `maybeRefreshToken`/`resolveWorkspaceAccessToken` into the credential proxy's `resolveCredential` callback, and (b) **supply the lock primitive** by passing its vault pool to the shared `pgAdvisoryLockAdapter` from `@hammies/auth` (the same helper studio uses, so the `pg_advisory_lock(hashtextextended(key, 0))` keyspace is identical across the two and they actually serialize against each other on the shared vault DB).

#### Scenario: The credential proxy's `resolveCredential` callback resolves by integration scope, not one fallback chain
- **WHEN** the credential proxy (`server/lib/credential-proxy.ts`) calls its `resolveCredential(sessionToken, integration)` callback (`server/index.ts`)
- **THEN** for a `scope: "workspace"` integration (looked up via `getIntegration(integration)`), the callback calls `resolveWorkspaceAccessToken(workspaceId, integration)` — which mints a short-lived scoped access token from the stored service-account key for a `service_account` row (e.g. `google`'s Analytics/Search Console SA, impersonating `impersonateSubjectEnv` when the integration declares one), refreshes a stored OAuth token for an `oauth2` row (e.g. `google-ads`), or returns a static key for an `api_key` row — never the raw stored value verbatim when that value isn't already a usable token.
- **AND** for a `scope: "user"` integration (e.g. `google-workspace`'s Gmail/Calendar/Sheets OAuth), the callback calls `maybeRefreshToken(session.user.email, integration)` first, falling back to the legacy `resolveCredential(email, workspaceId, integration)` vault lookup only if no OAuth-refreshable row exists.
- **AND** when the matched integration declares `proxy.headers` (config-backed extra headers, e.g. Google Ads' `developer-token`), the callback resolves each declared `valueEnv` via `resolveConfigVar(email, workspaceId, integration, valueEnv)` and returns it in `cred.extras`, keyed by `valueEnv` — resolved inside this trusted process so the raw value never rides in the agent's environment.
- **WHY:** the credential proxy's per-host mapping (see `credential-proxy` spec) resolves specific `*.googleapis.com` hosts to distinct integrations with different scopes and auth types. Before this, every one of them resolved through the same "return whatever string is stored" `resolveCredential` path — which returns a service-account JSON blob verbatim for the `google` row, unusable as a Bearer token.
...
| The credential proxy's `resolveCredential` callback — scope-based dispatch to `resolveWorkspaceAccessToken`/`maybeRefreshToken`, `extras` for Gorgias Basic auth and config-backed proxy headers | [server/index.ts](../../../server/index.ts) `createCredentialProxy({ resolveCredential })` |
| Workspace-scope, type-aware token resolution (service-account minting via `mintServiceAccountToken`, OAuth refresh, static keys) + config-var resolution | `@hammies/auth` `src/server/credentials/{refresh,service-account,vault}.ts` (owned by `credential-vault`); re-exported via [server/lib/vault.ts](../../../server/lib/vault.ts) as `resolveWorkspaceAccessToken`/`resolveConfigVar` |
...
- 2026-08-06: **`resolveCredential`'s callback now dispatches by integration scope instead of one `maybeRefreshToken ?? resolveCredential` chain for everything.** The credential proxy's per-host mapping (see `credential-proxy` spec) resolves specific `*.googleapis.com` hosts to distinct Google integrations, several of them `scope: "workspace"` and `authType: "service_account"` or `"oauth2"` — but the old callback only ever tried a *user*-scoped OAuth refresh, then fell back to the vault's raw stored value. For the `google` service-account row that raw value is a JSON key blob, injected verbatim as a Bearer token. Fixed by branching on `getIntegration(integration)?.scope`: `workspace` routes through `resolveWorkspaceAccessToken` (type-aware — mints, refreshes, or returns a static key as appropriate), `user` keeps the original `maybeRefreshToken` path. Also fixed the fallback's workspace key: it passed `workspacePaths[0]` (an absolute path) where `resolveWorkspaceAccessToken`/`seedWorkspaceCredentials` both key on `registeredWorkspaces[0].id` (`basename(path)`) — the mismatch meant the vault-lookup fallback for a workspace credential without an OAuth refresh token could never find the row `seedWorkspaceCredentials` had seeded. Also added `extras` resolution via `resolveConfigVar` for integrations that declare `proxy.headers` (Google Ads' `developer-token`). Credential-management plan, Phase A8.
