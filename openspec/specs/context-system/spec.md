...
- Body-extractor `USER_PROMPT_TEMPLATE` was replaced with a GEPA-optimized version (see `packages/optimizer/artifacts/body_extract_optimized.json`). On a 50-example holdout the LLM-judge score rose 75.97 → 83.56 (+7.59pp) with all four rubric criteria — precision, recall, type correctness, noise filter — improving by 6+pp and none regressing. The new prompt encodes the noise-filter rules that had been accumulating as folklore (skip venue names, skip routine-customer-service participants, prefer formal company names from domain abbreviations, drop signature-line brand portfolios, etc.).
